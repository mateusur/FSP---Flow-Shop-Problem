...
void Brute_force::brute_force_recursion(Matrix& data, int index, size_t n)
{
	int temp;
	if (index >= n) {
		temp = calculate_Cmax(data);
...
int Brute_force::calculate_Cmax(const Matrix& data, vector<unsigned> kolejnosc) {
	//static short number_of_calls = 0;
	unsigned tasks = data.size();
	unsigned machines = data[1].size() - 1;
...
int main() {
	...
	for (int i = 0; i < names->size(); i++) {
		...
		cout << "\nCzas iteracji dla klasy n= " << i + 1 << ": " << double(czas / n);
		czas = 0;
		for (int i = 0; i < n; i++) {
			auto start = std::chrono::steady_clock::now();
			//brute_force_recursion(data, 0, data.size());
			brute_force_iteration2(data);
			auto end = std::chrono::steady_clock::now();
			czas += std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
		}
